...
class UserController extends AdminController
{
    ...
    public function home(?array $data): void
    {
        ...
        if (!empty($data["s"])) {
            $s = str_search($data["s"]);
            echo json_encode(["redirect" => url("/app/users/home/{$s}/1")]);
            return;
        }
        ...
        if (!empty($data["search"]) && str_search($data["search"]) != "all") {
            ...
            if (!$users->count()) {
                ...
                redirect("/app/users/home");
            }
        }

        $all = ($search ?? "all");
        $pager = new Pager(url("/app/users/home/{$all}/"));
    ...
    public function user(?array $data): void
    {
        ...
        if (!empty($data["action"]) && $data["action"] == "create") {
            ...
            $json["redirect"] = url("/app/users/user/{$userCreate->id}");
